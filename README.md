...
`rag-api` 支持在聊天接口中执行受限 shell 命令，默认关闭。

先在 `.env` 中开启：

```env
ENABLE_SHELL_TOOL=true
SHELL_TOOL_WORKDIR=/kb
SHELL_TOOL_TIMEOUT_SECONDS=15
SHELL_TOOL_MAX_OUTPUT_CHARS=12000
```

说明：

- `SHELL_TOOL_WORKDIR` 默认是 `/kb`，命令会在这个目录下执行
- `DIRECTORY_ALIAS_MAP` 中配置过的别名可直接作为命令参数使用
- 默认禁止 `rm`、`mv`、`chmod`、`shutdown` 等高风险命令
- 建议保持知识库挂载为只读，避免误操作
...
如果你想先看目录里有什么资料，也可以直接提问：

- `维瞰视界有哪些文件`
- `列出维瞰视界目录下的文件`
- `天空卫士文件列表`

系统会直接返回该目录下已索引文件清单，方便再按文件名继续追问。
